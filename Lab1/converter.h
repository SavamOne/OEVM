...
template <class T>
T logic(const string& num, const unsigned short& ns)
{
	T res = 0;
	bool exceed = false;
	for (unsigned long long i = num.length(); i > 0; i--)
	{
		...
		{
			cout << "WARNING! Exceeding the maximum data type value, loss of accuracy" << endl;
			exceed = true;
		} 
	}
	return res;
...
template <class T>
T convert2decimal(const string& num, const unsigned short& ns)
{
	bool is_neg = (num[0] == '-');
	bool is_pos = (num[0] == '+');
	...
	if (split_index == -1)
		res = logic<T>(num.substr(start_index), ns);
	else
	{
		T int_value =  logic<T>(num.substr(start_index, split_index - start_index), ns);
